...
public class AVLTree<T extends Comparable<T>> {
    ...
    public boolean put(T element) {
        Entry<T> t = root;
        if (t == null) {
            root = new Entry<T>(element, null);
            size = 1;
            return true;
        }
        int cmp;
        Entry<T> parent;    //保存t的父节点
        ...
        Entry<T> child = new Entry(element, parent);
        if (cmp < 0) {
            parent.left = child;
     ...
     * 至于调整之后，各节点的BF变化见代码
     */
    private void fixAfterInsertion(Entry<T> p) {
        if (p.balance == 2) {
            leftBalance(p);
        }
        if (p.balance == -2) {
     ...
     * 情况4(L等高)
     */
    private boolean leftBalance(Entry<T> t) {
     ...
     * 情况4(r的BF为0)
     */
    private boolean rightBalance(Entry<T> t) {
        boolean heightLower = true;
        Entry<T> r = t.right;
        switch (r.balance) {
            case LH:            //左高，分情况调整
     ...
     */
    public Entry<T> getEntry(Object element) {
        Entry<T> tmp = root;
        Comparable<? super T> e = (Comparable<? super T>) element;
        int c;
        while (tmp != null) {
     ...
     */
    private void fixAfterDeletion(Entry<T> p) {
        boolean heightLower = true;        //看最小子树调整后，它的高度是否发生变化，如果减小，继续回溯
        Entry<T> t = p.parent;
        Comparable<? super T> e = (Comparable<? super T>) p.element;
        int cmp;
        //自下向上回溯，查找不平衡的节点进行调整
